...
test-prod: ## Test production build locally on port 8081
	docker build -t bauhaus-portfolio:test .
	docker run -d -p 8081:80 --name bauhaus-test bauhaus-portfolio:test
	@echo "Production build running at http://localhost:8081"
	@echo "Run 'docker stop bauhaus-test && docker rm bauhaus-test' to stop"
